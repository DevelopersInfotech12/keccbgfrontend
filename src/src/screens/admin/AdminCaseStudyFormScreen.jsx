...
import { C, SECTORS, EMPTY_CASE_STUDY } from "@/lib/adminConstants";

const TONES = ["leaf", "blush"];

export default function AdminCaseStudyFormScreen({ caseStudyId }) {
  const router = useRouter();
  const { admin, loading: authLoading, logout } = useAdminAuth();
  const [form, setForm] = useState(EMPTY_CASE_STUDY);
  const [fetchLoading, setFetchLoading] = useState(!!caseStudyId);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await adminApi.getCaseStudy(caseStudyId);
        const c = res.data;
        setForm({
          ...EMPTY_CASE_STUDY, ...c,
          highlights: c.highlights?.length ? c.highlights : [""],
          results: c.results?.length ? c.results : [{ value: "", label: "", tone: "leaf" }],
          toc: c.toc?.length ? c.toc : [{ id: "", label: "" }],
          meta: c.meta?.length ? c.meta : [{ label: "", value: "" }],
          sections: c.sections?.length ? c.sections : [{ id: "", heading: "", content: [{ type: "p", text: "" }] }],
          tags: c.tags?.length ? c.tags : [""],
          gallery: c.gallery?.length ? c.gallery : [],
          related: c.related || [],
          seo: { ...EMPTY_CASE_STUDY.seo, ...c.seo },
          sidebarCta: { ...EMPTY_CASE_STUDY.sidebarCta, ...c.sidebarCta },
        });
      } catch (e) { showToast("error", "Failed to load case study: " + e.message); }
      finally { setFetchLoading(false); }
    })();
  }, [caseStudyId, isEdit, authLoading]);
  ...
  const loadJson = (raw) => {
    ...
    try {
      ...
      if (typeof parsed !== "object" || Array.isArray(parsed)) { setJsonError("JSON must be an object matching the case-study schema."); return; }
      setForm((prev) => ({
        ...EMPTY_CASE_STUDY, ...prev, ...parsed,
        highlights: parsed.highlights?.length ? parsed.highlights : prev.highlights,
        results: parsed.results?.length ? parsed.results : prev.results,
        ...
        gallery: parsed.gallery?.length ? parsed.gallery : prev.gallery,
        related: parsed.related || prev.related,
        seo: { ...EMPTY_CASE_STUDY.seo, ...prev.seo, ...parsed.seo },
        sidebarCta: { ...EMPTY_CASE_STUDY.sidebarCta, ...prev.sidebarCta, ...parsed.sidebarCta },
      }));
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!form.title || !form.sector || !form.excerpt || !form.date || !form.img) {
      showToast("error", "Title, Sector, Excerpt, Date and Image are required.");
      return;
    }
    setSaving(true);
    try {
      ...
      if (isEdit) {
        await adminApi.updateCaseStudy(caseStudyId, payload);
        showToast("success", "Case study updated successfully!");
      } else {
        await adminApi.createCaseStudy(payload);
  ...
  return (
    <>
      <style>{`
        ...
        .img-upload-btn:hover { border-color:${C.coral}; color:${C.coral}; }
        .save-btn { display:inline-flex; align-items:center; gap:8px; padding:11px 24px; border:none; border-radius:8px; font-family:${C.sans}; font-size:14px; font-weight:600; cursor:pointer; background:linear-gradient(135deg,${C.coral},#D96450); color:#fff; transition:all 0.2s; }
        .save-btn:hover:not(:disabled) { transform:translateY(-1px); box-shadow:0 6px 20px rgba(236,124,98,0.3); }
        .save-btn:disabled { opacity:0.65; cursor:not-allowed; }
        .add-btn { font-size:12.5px; color:${C.coral}; background:none; border:none; cursor:pointer; padding:4px 0; }
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
        <Link href="/admin/case-studies" style={{ display: "flex", alignItems: "center", gap: 6, color: C.muted, textDecoration: "none", fontSize: 13 }}>
          <ArrowLeft size={15} />Back
        </Link>
        <h1 style={{ fontFamily: C.display, fontSize: 21, fontWeight: 700, color: C.text }}>{isEdit ? "Edit Case Study" : "Create New Case Study"}</h1>
        <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
          <button type="button" onClick={() => { setJsonPanel((o) => !o); setJsonError(""); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", border: `1.5px solid ${jsonPanel ? C.coral : C.border}`, borderRadius: 8, background: jsonPanel ? C.coralLight : "#fff", cursor: "pointer", fontFamily: C.sans, fontSize: 12.5, fontWeight: 600, color: jsonPanel ? C.coral : C.body }}>
          ...
          <button className="save-btn" onClick={handleSave} disabled={saving}>
            <Save size={15} />{saving ? "Saving…" : isEdit ? "Update Case Study" : "Create Case Study"}
          </button>
        </div>
      </div>

      {jsonPanel && (
        <div style={{ background: "#FEF6F4", border: `1.5px solid ${C.coral}33`, borderRadius: 14, padding: "22px 24px", marginBottom: 22, boxShadow: "0 4px 16px rgba(236,124,98,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <div>
              <div style={{ fontFamily: C.display, fontSize: 15, fontWeight: 700, color: C.text, display: "flex", alignItems: "center", gap: 8 }}>
                <FileJson size={18} color={C.coral} /> Upload Case Study Data (JSON)
              </div>
          ...
          <div style={{ display: "flex", marginBottom: 14, border: `1.5px solid ${C.border}`, borderRadius: 9, overflow: "hidden", background: "#fff", marginTop: 14 }}>
            {[{ key: "paste", icon: ClipboardPaste, label: "Paste JSON" }, { key: "file", icon: FolderOpen, label: "Upload File" }].map(({ key, icon: Icon, label }) => (
              <button key={key} type="button" onClick={() => setJsonTab(key)} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 0", border: "none", cursor: "pointer", fontFamily: C.sans, fontSize: 13, fontWeight: 600, background: jsonTab === key ? C.coral : "#fff", color: jsonTab === key ? "#fff" : C.muted }}>
                <Icon size={15} />{label}
              </button>
            ))}
          </div>
          ...
          {jsonTab === "file" && (
            <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 160, border: `2px dashed ${C.border}`, borderRadius: 10, background: "#fff", cursor: "pointer", gap: 10 }}>
              ...
              {jsonText && <div style={{ fontSize: 12, color: C.coral, fontWeight: 500 }}>✓ File loaded — click &quot;Load JSON Data&quot; below</div>}
          ...
          <button type="button" onClick={() => loadJson(jsonText)} disabled={!jsonText.trim()} style={{ width: "100%", marginTop: 14, padding: "12px", background: jsonText.trim() ? `linear-gradient(135deg,${C.coral},#D96450)` : "#CBD5E0", color: "#fff", border: "none", borderRadius: 9, fontFamily: C.sans, fontSize: 14, fontWeight: 600, cursor: jsonText.trim() ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      <form onSubmit={handleSave} onKeyDown={(e) => e.key === "Enter" && e.target.tagName !== "TEXTAREA" && e.preventDefault()}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 18, alignItems: "start" }}>
          <div>
            <SectionCard title="Core Information">
              <div style={{ display: "grid", gap: 14, marginTop: 14 }}>
                <div>
                  ...
                  <Input value={form.title} onChange={(e) => { set("title", e.target.value); if (!isEdit) set("slug", autoSlug(e.target.value)); }} placeholder="Case study title…" />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div><Label required>Slug</Label><Input value={form.slug} onChange={(e) => set("slug", e.target.value)} placeholder="case-study-slug" /></div>
                  <div>
                    <Label required>Sector</Label>
                    <select value={form.sector} onChange={(e) => set("sector", e.target.value)} style={{ width: "100%", padding: "9px 12px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontFamily: C.sans, fontSize: 13, color: C.text, outline: "none" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div><Label>Client</Label><Input value={form.client} onChange={(e) => set("client", e.target.value)} placeholder="Client name (or 'name withheld under NDA')" /></div>
                  <div><Label>Location</Label><Input value={form.location} onChange={(e) => set("location", e.target.value)} placeholder="Anand, Gujarat" /></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  <div><Label>Capacity</Label><Input value={form.capacity} onChange={(e) => set("capacity", e.target.value)} placeholder="12 t/day CBG" /></div>
                  <div><Label required>Date</Label><Input value={form.date} onChange={(e) => set("date", e.target.value)} placeholder="March 2026" /></div>
                  <div><Label>Read Time</Label><Input value={form.readTime} onChange={(e) => set("readTime", e.target.value)} placeholder="4 min read" /></div>
                </div>
              </div>
            </SectionCard>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {form.gallery.map((src, i) => (
                      <div key={i} style={{ display: "flex", gap: 8 }}>
                        <Input value={src} onChange={(e) => arrUpdate("gallery", i, e.target.value)} placeholder="Image URL" style={{ flex: 1 }} />
                        <label className="img-upload-btn" title="Upload image">
                          {imgUploading[`gallery-${i}`] ? "…" : <Upload size={13} />}
                          <input type="file" accept="image/*" style={{ display: "none" }} onChange={(e) => handleImgUpload(`gallery-${i}`, e.target.files[0], (url) => arrUpdate("gallery", i, url))} />
                        </label>
                        <button type="button" onClick={() => arrRemove("gallery", i)} style={{ background: "none", border: "none", cursor: "pointer", color: "#EF4444", flexShrink: 0 }}><Trash2 size={14} /></button>
                      </div>
                    ))}
                    <button type="button" className="add-btn" onClick={() => arrAdd("gallery", "")}>+ Add gallery image</button>
                  </div>
                </div>
              </div>
            </SectionCard>

            <SectionCard title="Highlights (At a Glance)">
            ...
            <SectionCard title="Result Stats (homepage & hero plinths)">
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
                {form.results.map((r, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 2fr 110px auto", gap: 8 }}>
                    <Input value={r.value} onChange={(e) => arrUpdate("results", i, { ...r, value: e.target.value })} placeholder="70%" />
                    <Input value={r.label} onChange={(e) => arrUpdate("results", i, { ...r, label: e.target.value })} placeholder="of site gas demand met" />
                    <select value={r.tone} onChange={(e) => arrUpdate("results", i, { ...r, tone: e.target.value })} style={{ padding: "9px 8px", borderRadius: 8, border: `1.5px solid ${C.border}`, fontFamily: C.sans, fontSize: 12.5, color: C.text }}>
                      {TONES.map((t) => <option key={t} value={t}>{t}</option>)}
                    </select>
                    <button type="button" onClick={() => arrRemove("results", i)} style={{ background: "none", border: "none", cursor: "pointer", color: "#EF4444" }}><Trash2 size={14} /></button>
                  </div>
                ))}
                <button type="button" className="add-btn" onClick={() => arrAdd("results", { value: "", label: "", tone: "leaf" })}>+ Add result stat</button>
              </div>
            </SectionCard>
            ...
            <SectionCard title="Project Info (Sidebar)" defaultOpen={false}>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 14 }}>
                {form.meta.map((m, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 8 }}>
            ...
            <SectionCard title="CTA Band" defaultOpen={false}>
              <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
                <div><Label>CTA Title</Label><Input value={form.ctaTitle} onChange={(e) => set("ctaTitle", e.target.value)} placeholder="Want results like this on your site?" /></div>
            ...
            <SectionCard title="SEO Settings" defaultOpen={false}>
              <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
                <div><Label>Meta Title</Label><Input value={form.seo.metaTitle} onChange={(e) => setNested("seo", "metaTitle", e.target.value)} placeholder="Defaults to title" /></div>
                ...
                <div><Label>Meta Keywords (comma-separated)</Label><Input value={(form.seo.metaKeywords || []).join(", ")} onChange={(e) => setNested("seo", "metaKeywords", e.target.value.split(",").map((s) => s.trim()))} placeholder="CBG, case study, biogas plant…" /></div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div><Label>Canonical URL</Label><Input value={form.seo.canonicalUrl} onChange={(e) => setNested("seo", "canonicalUrl", e.target.value)} placeholder="https://biocbg.com/case-studies/…" /></div>
          ...
          <div>
            <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "18px 18px", boxShadow: "0 2px 8px rgba(10,19,16,0.04)", marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "#FEF3DC", borderRadius: 8, border: "1px solid #FDE68A" }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#92400E" }}>★ Featured on Homepage</div>
                    <div style={{ fontSize: 11.5, color: "#B45309", marginTop: 1 }}>Only 1 case study can be featured at a time</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "18px 18px", boxShadow: "0 2px 8px rgba(10,19,16,0.04)", marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div><Label>Title</Label><Input value={form.sidebarCta.title} onChange={(e) => setNested("sidebarCta", "title", e.target.value)} placeholder="Considering something similar?" /></div>
